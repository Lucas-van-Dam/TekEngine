...
void Model::loadModel(string const &path, const std::shared_ptr<GameObject> &parent) {
...
std::shared_ptr<GameObject> Model::processMesh(aiMesh *mesh, const aiScene *scene) {
    ...
    std::shared_ptr<Material> material = make_shared<Material>(make_shared<Shader>("PBR.vert", "PBR.frag"));
...
std::shared_ptr<Texture> Model::loadMaterialTextures(aiMaterial *mat, aiTextureType type, const string &typeName) {
